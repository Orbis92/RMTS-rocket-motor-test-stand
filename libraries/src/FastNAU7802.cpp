...
int32_t NAU7802::getReading()
{
  ...
  {
    uint32_t valueRaw = (uint32_t)_i2cPort->read() << 8;   //  << 16;    //MSB
    valueRaw |= (uint32_t)_i2cPort->read() ;               //  <<  8;     //MidSB
    //valueRaw |= (uint32_t)_i2cPort->read()  ;            // (<<  0);    //LSB
    _i2cPort->read();  //dump the last 8 bits (if commented out above)
	...
	int32_t valueShifted = (int32_t)(valueRaw << 16);

	// shift the number back right to recover its intended magnitude
	int32_t value = ( valueShifted >> 16 );
